...
public partial class Program
{
    public static void Main()
    {
        ...
        var holy = info.GetHoliDeInfo(out var total, year, null, 100);
        Console.WriteLine($"------- 국경일: {total}");
        foreach (var day in holy)
            Console.WriteLine(day.ToString());
        Console.WriteLine("");

        var rest = info.GetRestDeInfo(out total, year, null, 100);
        Console.WriteLine($"------- 공휴일: {total}");
        foreach (var day in rest)
            Console.WriteLine(day.ToString());
        Console.WriteLine("");

        var anniv = info.GetAnniversaryInfo(out total, year, null, 100);
        Console.WriteLine($"------- 기념일: {total}");
        foreach (var day in anniv)
            Console.WriteLine(day.ToString());
        Console.WriteLine("");

        var div24 = info.Get24DivisionsInfo(out total, year, null, 100);
        Console.WriteLine($"------- 24절기: {total}");
        foreach (var day in div24)
            Console.WriteLine(day.ToString());
        Console.WriteLine("");

        var sundry = info.GetSundryDayInfo(out total, year, null, 100);
